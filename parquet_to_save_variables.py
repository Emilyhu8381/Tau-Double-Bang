# this script is to enable waveforms to be saved in a parquet file 
# this script is to test whether a parquet file can be created and called
import numpy as np
...
a = ak.from_parquet(f"/n/home06/qhu/tau_sim/output/111_photons.parquet")
...
for i in lep_event_id:
    ...
    for j in domids:
        if len(d[j]) > 30 and len(d[j]) < 1500: # contrain photon_len before creating instances 
...
pa.parquet.write_table(trigger_events, "111_trigger_events.parquet")
